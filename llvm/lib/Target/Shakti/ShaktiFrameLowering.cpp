//===-- ShaktiFrameLowering.cpp - Shakti Frame Information -----------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Shakti implementation of TargetFrameLowering
// class.
//
//===----------------------------------------------------------------------===//
...
void ShaktiFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  // fp = sp
  if (hasFP(MF)) {
    BuildMI(MBB, MBBI, DL, TII.get(Shakti::MOVESS))
        .addReg(Shakti::FP_REG)
        .addReg(Shakti::SP_REG);

    // emit ".cfi_def_cfa_register $fp" (debug information)
    unsigned CFIIndex = MF.addFrameInst(MCCFIInstruction::createDefCfaRegister(
        nullptr, MRI->getDwarfRegNum(Shakti::FP_REG, true)));
    BuildMI(MBB, MBBI, DL, TII.get(TargetOpcode::CFI_INSTRUCTION))
        .addCFIIndex(CFIIndex);
  }
}

void ShaktiFrameLowering::emitEpilogue(MachineFunction &MF,
                                      MachineBasicBlock &MBB) const {
  ...
  assert(MBBI->getOpcode() == Shakti::RET &&
  ...
  if (hasFP(MF)) {
    ...
    BuildMI(MBB, I, DL, TII.get(Shakti::MOVESS))
        .addReg(Shakti::SP_REG)
        .addReg(Shakti::FP_REG);
  }
...
MachineBasicBlock::iterator ShaktiFrameLowering::eliminateCallFramePseudoInstr(
  ...
  if (Amount != 0 && !hasReservedCallFrame(MF)) {
    ...
    if (MI.getOpcode() == Shakti::ADJCALLSTACKDOWN)
      Amount = -Amount;
...
void ShaktiFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                              BitVector &SavedRegs,
                                              RegScavenger *RS) const {

  TargetFrameLowering::determineCalleeSaves(MF, SavedRegs, RS);
  if (hasFP(MF))
    SavedRegs.set(Shakti::FP_REG);
  ...
  // ShaktiRegisterInfo::eliminateFrameIndex
  if (isInt<14>(getWorstCaseStackSize(MF)))
    return;

  const TargetRegisterInfo *TRI = MF.getSubtarget().getRegisterInfo();
  const TargetRegisterClass &RC = Shakti::GPR32RegClass;
...
bool ShaktiFrameLowering::hasFP(const MachineFunction &MF) const {
...
// arguments to called.
bool ShaktiFrameLowering::hasReservedCallFrame(const MachineFunction &MF) const {
  return !MF.getFrameInfo().hasVarSizedObjects();
}

uint64_t
ShaktiFrameLowering::getWorstCaseStackSize(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const TargetRegisterInfo &TRI = *MF.getSubtarget().getRegisterInfo();
