//===-- MipsAsmPrinter.h - Mips LLVM Assembly Printer ----------*- C++ -*--===//
//
//                     The LLVM Compiler Infrastructure
//
// This file is distributed under the University of Illinois Open Source
// License. See LICENSE.TXT for details.
//
//===----------------------------------------------------------------------===//
//
// Shakti Assembly printer class.
//
// The name is a bit misleading.  Because we use the MC layer for code
// generation, the job of this class is now mostly to convert MachineInstrs
// into MCInsts. Most of the work is done by a helper class MCInstLowering
// (which in turn uses code generated by TableGen). This also performs
// replacement of inline assembly parameters.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_SHAKTI_SHAKTIASMPRINTER_H
#define LLVM_LIB_TARGET_SHAKTI_SHAKTIASMPRINTER_H

#include "ShaktiMCInstLower.h"
#include "llvm/CodeGen/AsmPrinter.h"
#include "llvm/MC/MCStreamer.h"
#include "llvm/Support/Compiler.h"

namespace llvm {
class MachineInstr;
class MachineBasicBlock;
class TargetMachine;
class raw_ostream;

class LLVM_LIBRARY_VISIBILITY ShaktiAsmPrinter : public AsmPrinter {
  ShaktiMCInstLower MCInstLowering;

public:
  explicit ShaktiAsmPrinter(TargetMachine &TM,
                           std::unique_ptr<MCStreamer> Streamer)
      : AsmPrinter(TM, std::move(Streamer)), MCInstLowering(*this) {}

  StringRef getPassName() const override { return "Shakti Assembly Printer"; }

  void EmitInstruction(const MachineInstr *MI) override;
  void EmitFunctionBodyStart() override;

  // Print operand for inline assembly
  bool PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
                       unsigned AsmVariant, const char *ExtraCode,
                       raw_ostream &O) override;
  bool PrintAsmMemoryOperand(const MachineInstr *MI, unsigned OpNum,
                             unsigned AsmVariant, const char *ExtraCode,
                             raw_ostream &O) override;
};

} // namespace llvm

#endif
