//===-- ShaktiISelLowering.cpp - Shakti DAG Lowering Implementation ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the interfaces that Shakti uses to lower LLVM code
// into a selection DAG.
//
//===----------------------------------------------------------------------===//

#define DEBUG_TYPE "shakti-isel-lowering"

#include "ShaktiISelLowering.h"
#include "MCTargetDesc/ShaktiBaseInfo.h"
#include "MCTargetDesc/ShaktiMCTargetDesc.h"
#include "ShaktiMachineFunctionInfo.h"
...
#include "ShaktiGenCallingConv.inc"
...
SDValue morphSETCCNode(SDValue Op, ShaktiISD::NodeType Compare, SelectionDAG &DAG) {
  SDLoc DL(Op);
...
ShaktiTargetLowering::ShaktiTargetLowering(const TargetMachine &TM,
                                         const ShaktiSubtarget &STI)
    : TargetLowering(TM), Subtarget(STI) {

  // Set up the register classes.
  addRegisterClass(MVT::i32, &Shakti::GPR32RegClass);
  addRegisterClass(MVT::v16i1, &Shakti::GPR32RegClass);
  addRegisterClass(MVT::f32, &Shakti::GPR32RegClass);
  addRegisterClass(MVT::v16i32, &Shakti::VR512RegClass);
  addRegisterClass(MVT::v16f32, &Shakti::VR512RegClass);
  ...
  setStackPointerRegisterToSaveRestore(Shakti::SP_REG);
  setMinFunctionAlignment(2);

  computeRegisterProperties(Subtarget.getRegisterInfo());
}

SDValue ShaktiTargetLowering::LowerOperation(SDValue Op,
...
SDValue
ShaktiTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                 bool IsVarArg,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Shakti32);
  ...
  if (MF.getFunction()->hasStructRetAttr()) {
    ShaktiMachineFunctionInfo *VFI = MF.getInfo<ShaktiMachineFunctionInfo>();
    ...
    Chain = DAG.getCopyToReg(Chain, DL, Shakti::S0, Val, Flag);
    Flag = Chain.getValue(1);
    RetOps.push_back(
        DAG.getRegister(Shakti::S0, getPointerTy(DAG.getDataLayout())));
  }
  ...
  return DAG.getNode(ShaktiISD::RET_FLAG, DL, MVT::Other, RetOps);
}

SDValue ShaktiTargetLowering::LowerFormalArguments(
  ...
  // ShaktiCallingConv.td will auto-generate CC_Shakti32, which
  // knows how to handle operands (what go in registers vs. stack, etc).
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Shakti32);
  ...
  for (const auto &VA : ArgLocs) {
    if (VA.isRegLoc()) {
      ...
      if (RegVT == MVT::i32 || RegVT == MVT::f32 || RegVT == MVT::v16i1)
        RC = &Shakti::GPR32RegClass;
      else if (RegVT == MVT::v16i32 || RegVT == MVT::v16f32)
        RC = &Shakti::VR512RegClass;
      else
  ...
  if (MF.getFunction()->hasStructRetAttr()) {
    ...
    unsigned Reg = VFI->getSRetReturnReg();
    if (!Reg) {
      Reg = MF.getRegInfo().createVirtualRegister(&Shakti::GPR32RegClass);
...
SDValue ShaktiTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // Analyze operands of the call, assigning locations to each operand.
  // ShaktiCallingConv.td will auto-generate CC_Shakti32, which knows how to
  // handle operands (what go in registers vs. stack, etc).
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_Shakti32);
  ...
  for (unsigned i = 0, realArgIdx = 0, byvalArgIdx = 0, e = ArgLocs.size();
    ...
    SDValue StackPtr = DAG.getRegister(Shakti::SP_REG, MVT::i32);
  ...
  Chain = DAG.getNode(ShaktiISD::CALL, DL, NodeTys, Ops);
  InFlag = Chain.getValue(1);
  ...
  RVInfo.AnalyzeCallResult(Ins, RetCC_Shakti32);
...
MachineBasicBlock *
ShaktiTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
                                                 MachineBasicBlock *BB) const {
  switch (MI.getOpcode()) {
  case Shakti::SELECTI:
  case Shakti::SELECTF:
  case Shakti::SELECTVI:
  case Shakti::SELECTVF:
  case Shakti::SELECTMASK:
    return EmitSelectCC(MI, BB);

  case Shakti::ATOMIC_LOAD_ADDR:
    return EmitAtomicBinary(MI, BB, Shakti::ADDISSS);

  case Shakti::ATOMIC_LOAD_ADDI:
    return EmitAtomicBinary(MI, BB, Shakti::ADDISSI);

  case Shakti::ATOMIC_LOAD_SUBR:
    return EmitAtomicBinary(MI, BB, Shakti::SUBISSS);

  case Shakti::ATOMIC_LOAD_NANDR:
    return EmitAtomicBinary(MI, BB, Shakti::ANDSSS, true);

  case Shakti::ATOMIC_LOAD_SUBI:
    return EmitAtomicBinary(MI, BB, Shakti::SUBISSI);

  case Shakti::ATOMIC_LOAD_ANDR:
    return EmitAtomicBinary(MI, BB, Shakti::ANDSSS);

  case Shakti::ATOMIC_LOAD_ANDI:
    return EmitAtomicBinary(MI, BB, Shakti::ANDSSI);

  case Shakti::ATOMIC_LOAD_ORR:
    return EmitAtomicBinary(MI, BB, Shakti::ORSSS);

  case Shakti::ATOMIC_LOAD_ORI:
    return EmitAtomicBinary(MI, BB, Shakti::ORSSI);

  case Shakti::ATOMIC_LOAD_XORR:
    return EmitAtomicBinary(MI, BB, Shakti::XORSSS);

  case Shakti::ATOMIC_LOAD_XORI:
    return EmitAtomicBinary(MI, BB, Shakti::XORSSI);

  case Shakti::ATOMIC_LOAD_NANDI:
    return EmitAtomicBinary(MI, BB, Shakti::ANDSSI, true);

  case Shakti::ATOMIC_SWAP:
    return EmitAtomicBinary(MI, BB, 0);

  case Shakti::ATOMIC_CMP_SWAP:
...
const char *ShaktiTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case ShaktiISD::CALL:
    return "ShaktiISD::CALL";
  case ShaktiISD::RET_FLAG:
    return "ShaktiISD::RET_FLAG";
  case ShaktiISD::SPLAT:
    return "ShaktiISD::SPLAT";
  case ShaktiISD::SEL_COND_RESULT:
    return "ShaktiISD::SEL_COND_RESULT";
  case ShaktiISD::RECIPROCAL_EST:
    return "ShaktiISD::RECIPROCAL_EST";
  case ShaktiISD::MASK_TO_INT:
    return "ShaktiISD::MASK_TO_INT";
  case ShaktiISD::MASK_FROM_INT:
    return "ShaktiISD::MASK_FROM_INT";
  case ShaktiISD::MOVEHI:
    return "ShaktiISD::MOVEHI";
  case ShaktiISD::ORLO:
    return "ShaktiISD::ORLO";
  case ShaktiISD::FGT:
    return "ShaktiISD::FGT";
  case ShaktiISD::FGE:
    return "ShaktiISD::FGE";
  case ShaktiISD::FLT:
    return "ShaktiISD::FLT";
  case ShaktiISD::FLE:
    return "ShaktiISD::FLE";
  case ShaktiISD::FEQ:
    return "ShaktiISD::FEQ";
  case ShaktiISD::FNE:
    return "ShaktiISD::FNE";
  default:
    return nullptr;
  }
}
...
ShaktiTargetLowering::ConstraintType
ShaktiTargetLowering::getConstraintType(StringRef Constraint) const {
...
ShaktiTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    ...
    case 's':
    case 'r':
      return std::make_pair(0U, &Shakti::GPR32RegClass);

    case 'v':
      return std::make_pair(0U, &Shakti::VR512RegClass);
    }
  }
...
bool ShaktiTargetLowering::isOffsetFoldingLegal(
    const GlobalAddressSDNode *GA) const {
  // The Shakti target isn't yet aware of offsets.
  return false;
}

EVT ShaktiTargetLowering::getSetCCResultType(const DataLayout &,
                                            LLVMContext &Context,
                                            EVT VT) const {
  if (VT.isVector()) {
...
// file.
bool ShaktiTargetLowering::isShuffleMaskLegal(const SmallVectorImpl<int> &M,
                                             EVT VT) const {
  if (M.size() != 16)
    return false;
...
bool ShaktiTargetLowering::isIntDivCheap(EVT, AttributeList) const {
  return false;
}

bool ShaktiTargetLowering::shouldInsertFencesForAtomic(
    const Instruction *I) const {
  return true;
}

SDValue ShaktiTargetLowering::getTargetNode(const GlobalAddressSDNode *N, EVT Ty,
                                           SelectionDAG &DAG,
                                           unsigned int Flag) const {
  return DAG.getTargetGlobalAddress(N->getGlobal(), SDLoc(N), Ty, 0, Flag);
}

SDValue ShaktiTargetLowering::getTargetNode(const JumpTableSDNode *N, EVT Ty,
                                           SelectionDAG &DAG,
                                           unsigned int Flag) const {
  return DAG.getTargetJumpTable(N->getIndex(), Ty, Flag);
}

SDValue ShaktiTargetLowering::getTargetNode(const ConstantPoolSDNode *N, EVT Ty,
                                           SelectionDAG &DAG,
                                           unsigned int Flag) const {
...
SDValue ShaktiTargetLowering::getTargetNode(const BlockAddressSDNode *N, EVT Ty,
                                           SelectionDAG &DAG,
                                           unsigned int Flag) const {
...
SDValue ShaktiTargetLowering::getAddr(const NodeTy *N, SelectionDAG &DAG) const {
  SDLoc DL(N);
  EVT Ty = getPointerTy(DAG.getDataLayout());
  SDValue Hi = getTargetNode(N, Ty, DAG, Shakti::MO_ABS_HI);
  SDValue Lo = getTargetNode(N, Ty, DAG, Shakti::MO_ABS_LO);
  SDValue MoveHi = DAG.getNode(ShaktiISD::MOVEHI, DL, MVT::i32, Hi);
  return DAG.getNode(ShaktiISD::ORLO, DL, MVT::i32, MoveHi, Lo);
}

SDValue ShaktiTargetLowering::LowerGlobalAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  const GlobalAddressSDNode *N = cast<GlobalAddressSDNode>(Op);
  return getAddr(N, DAG);
}

SDValue ShaktiTargetLowering::LowerJumpTable(SDValue Op,
                                            SelectionDAG &DAG) const {
  JumpTableSDNode *N = cast<JumpTableSDNode>(Op);
  return getAddr(N, DAG);
}

SDValue ShaktiTargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
  ConstantPoolSDNode *N = cast<ConstantPoolSDNode>(Op);
  return getAddr(N, DAG);
}

SDValue ShaktiTargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  const BlockAddressSDNode *N = cast<BlockAddressSDNode>(Op);
  return getAddr(N, DAG);
}

SDValue ShaktiTargetLowering::LowerBUILD_VECTOR(SDValue Op,
                                               SelectionDAG &DAG) const {
  ...
  if (VT != MVT::v16i1) {
    if (isSplatVector(Op.getNode())) {
      ...
      return DAG.getNode(ShaktiISD::SPLAT, DL, VT, Op.getOperand(0));
    }
  ...
  if (ISD::isBuildVectorOfConstantSDNodes(Op.getNode())) {
    ...
    return DAG.getNode(ShaktiISD::MASK_FROM_INT, DL, MVT::v16i1, BitsConstant);
  } else {
    ...
    return DAG.getNode(ShaktiISD::MASK_FROM_INT, DL, MVT::v16i1, Bitmask);
  }
}

// VECTOR_SHUFFLE(vec1, vec2, shuffle_indices)
SDValue ShaktiTargetLowering::LowerVECTOR_SHUFFLE(SDValue Op,
                                                 SelectionDAG &DAG) const {
  MVT VT = Op.getValueType().getSimpleVT();
  ...
  if (Op.getOperand(0).getOpcode() == ISD::SCALAR_TO_VECTOR && Mask == 0)
    return DAG.getNode(ShaktiISD::SPLAT, DL, VT, Op.getOperand(0).getOperand(0));
  ...
  if (IsSplat) {
    ...
    if (Op.getOperand(0).getOpcode() == ISD::INSERT_VECTOR_ELT &&
        SplatIndex ==
            dyn_cast<ConstantSDNode>(Op.getOperand(0).getOperand(2))
                ->getSExtValue()) {
      return DAG.getNode(ShaktiISD::SPLAT, DL, VT,
    ...
    return DAG.getNode(ShaktiISD::SPLAT, DL, VT, LaneValue);
  }
  ...
  SDValue NativeShuffleIntr = DAG.getConstant(VT.isFloatingPoint()
    ? Intrinsic::shakti_shufflef : Intrinsic::shakti_shufflei, DL, MVT::i32);
  if (Mask == 0xffff || Mask == 0) {
    ...
    SDValue MaskVal = DAG.getNode(ShaktiISD::MASK_FROM_INT,
...
// (VECTOR, VAL, IDX)
SDValue ShaktiTargetLowering::LowerINSERT_VECTOR_ELT(SDValue Op,
                                                    SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (VT == MVT::v16i1) {
    ...
    SDValue VecAsInt =
        DAG.getNode(ShaktiISD::MASK_TO_INT, DL, MVT::i32, Op.getOperand(0));
    SDValue Result =
        DAG.getNode(ISD::OR, DL, MVT::i32, SetBit,
                    DAG.getNode(ISD::AND, DL, MVT::i32, VecAsInt, UnsetBit));
    return DAG.getNode(ShaktiISD::MASK_FROM_INT, DL, MVT::v16i1, Result);
  } else {
    // Put the element into the right lane with a masked move.
    SDValue Splat = DAG.getNode(ShaktiISD::SPLAT, DL, VT, Op.getOperand(1));
    SDValue MaskVec =
        DAG.getNode(ShaktiISD::MASK_FROM_INT, DL, MVT::v16i1, LaneMask);
    return DAG.getNode(ISD::VSELECT, DL, VT, MaskVec, Splat,
                       Op.getOperand(0));
  }
}

SDValue ShaktiTargetLowering::LowerEXTRACT_VECTOR_ELT(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  SDValue MaskBits = DAG.getNode(ShaktiISD::MASK_TO_INT, DL, MVT::i32, Vector);
...
SDValue ShaktiTargetLowering::LowerSCALAR_TO_VECTOR(SDValue Op,
                                                   SelectionDAG &DAG) const {
  MVT VT = Op.getValueType().getSimpleVT();
  SDLoc DL(Op);
  ...
  if (VT == MVT::v16i1) {
    ...
    return DAG.getNode(ShaktiISD::MASK_FROM_INT, DL, MVT::i32, Op.getOperand(0));
  } else {
...
// transformed.
SDValue ShaktiTargetLowering::LowerSELECT_CC(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  return DAG.getNode(ShaktiISD::SEL_COND_RESULT, DL, Op.getValueType(), Pred,
...
SDValue ShaktiTargetLowering::LowerSELECT(SDValue Op,
                                         SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue ShaktiTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  ShaktiISD::NodeType ComplementCompare;
  switch (CC) {
  default:
  ...
  case ISD::SETGT:
  case ISD::SETOGT:
    return morphSETCCNode(Op, ShaktiISD::FGT, DAG);
  case ISD::SETGE:
  case ISD::SETOGE:
    return morphSETCCNode(Op, ShaktiISD::FGE, DAG);
  case ISD::SETLT:
  case ISD::SETOLT:
    return morphSETCCNode(Op, ShaktiISD::FLT, DAG);
  case ISD::SETLE:
  case ISD::SETOLE:
    return morphSETCCNode(Op, ShaktiISD::FLE, DAG);
  case ISD::SETEQ:
  case ISD::SETOEQ:
    return morphSETCCNode(Op, ShaktiISD::FEQ, DAG);
  case ISD::SETNE:
  case ISD::SETONE:
    return morphSETCCNode(Op, ShaktiISD::FNE, DAG);
  ...
  case ISD::SETUO: {
    ...
    SDValue IsOrdered =
        DAG.getNode(ShaktiISD::FEQ, DL, ResultVT, Op0, Op0);
    if (CC == ISD::SETO)
      return IsOrdered;
    ...
    if (ResultVT.isVector()) {
      Negate = DAG.getNode(ShaktiISD::MASK_FROM_INT, DL, MVT::v16i1, Negate);
    }
  ...
  case ISD::SETUEQ:
    ComplementCompare = ShaktiISD::FNE;
    break;
  case ISD::SETUGT:
    ComplementCompare = ShaktiISD::FLE;
    break;
  case ISD::SETUGE:
    ComplementCompare = ShaktiISD::FLT;
    break;
  case ISD::SETULT:
    ComplementCompare = ShaktiISD::FGE;
    break;
  case ISD::SETULE:
    ComplementCompare = ShaktiISD::FGT;
    break;
  case ISD::SETUNE:
    ComplementCompare = ShaktiISD::FEQ;
    break;
  }
...
SDValue ShaktiTargetLowering::LowerFDIV(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  SDValue Estimate =
      DAG.getNode(ShaktiISD::RECIPROCAL_EST, DL, Type, Denominator);
...
// 0x80000000
SDValue ShaktiTargetLowering::LowerFNEG(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  SDValue iconv;
  if (ResultVT.isVector())
    rhs = DAG.getNode(ShaktiISD::SPLAT, DL, MVT::v16i32, rhs);
...
SDValue ShaktiTargetLowering::LowerFABS(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT ResultVT = Op.getValueType().getSimpleVT();
  MVT IntermediateVT = ResultVT.isVector() ? MVT::v16i32 : MVT::i32;
...
SDValue ShaktiTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MachineFunction &MF = DAG.getMachineFunction();
  ShaktiMachineFunctionInfo *VFI = MF.getInfo<ShaktiMachineFunctionInfo>();
...
SDValue ShaktiTargetLowering::LowerCTLZ_ZERO_UNDEF(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDLoc DL(Op);
  return DAG.getNode(ISD::CTLZ, DL, Op.getValueType(), Op.getOperand(0));
}

SDValue ShaktiTargetLowering::LowerCTTZ_ZERO_UNDEF(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDLoc DL(Op);
  return DAG.getNode(ISD::CTTZ, DL, Op.getValueType(), Op.getOperand(0));
}

SDValue ShaktiTargetLowering::LowerUINT_TO_FP(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  SDValue FalseVal = DAG.getNode(ShaktiISD::SPLAT, DL, MVT::v16f32,
                                 DAG.getConstantFP(0.0, DL, ElementType));
  SDValue TrueVal = DAG.getNode(ShaktiISD::SPLAT, DL, MVT::v16f32,
                                DAG.getConstantFP(1.0, DL, ElementType));
  return DAG.getNode(ISD::VSELECT, DL, MVT::v16f32, Op.getOperand(0),
                     TrueVal, FalseVal);
}

SDValue ShaktiTargetLowering::LowerSINT_TO_FP(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue ShaktiTargetLowering::LowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
  ...
  return DAG.getCopyFromReg(DAG.getEntryNode(), DL, Shakti::FP_REG, VT);
}

SDValue ShaktiTargetLowering::LowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  ...
  unsigned Reg = MF.addLiveIn(Shakti::RA_REG, getRegClassFor(VT));
...
SDValue ShaktiTargetLowering::LowerSIGN_EXTEND(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);

  SDValue FalseVal = DAG.getNode(ShaktiISD::SPLAT, DL, MVT::v16i32,
                                 DAG.getConstant(0, DL, MVT::i32));
  SDValue TrueVal = DAG.getNode(ShaktiISD::SPLAT, DL, MVT::v16i32,
...
SDValue ShaktiTargetLowering::LowerZERO_EXTEND(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue ShaktiTargetLowering::LowerTRUNCATE(SDValue Op,
                                           SelectionDAG &DAG) const {
  SDLoc DL(Op);

  SDValue Op0 = Op.getOperand(0);
  SDValue OnesVec = DAG.getNode(ShaktiISD::SPLAT, DL, MVT::v16i32,
                                DAG.getConstant(1, DL, MVT::i32));
...
// here.
SDValue ShaktiTargetLowering::LowerFP_TO_XINT(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue Zero = DAG.getNode(ShaktiISD::SPLAT, DL, MVT::v16f32,
                             DAG.getConstantFP(0.0, DL, MVT::f32));
  return DAG.getSetCC(DL, MVT::v16i1, Op.getOperand(0), Zero, ISD::SETNE);
}

MachineBasicBlock *
ShaktiTargetLowering::EmitSelectCC(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  ...
  BuildMI(BB, DL, TII->get(Shakti::BNZ))
  ...
  BuildMI(*BB, BB->begin(), DL, TII->get(Shakti::PHI), MI.getOperand(0).getReg())
...
MachineBasicBlock *
ShaktiTargetLowering::EmitAtomicBinary(MachineInstr &MI, MachineBasicBlock *BB,
  ...
  unsigned OldValue = MRI.createVirtualRegister(&Shakti::GPR32RegClass);
  unsigned Success = MRI.createVirtualRegister(&Shakti::GPR32RegClass);
  ...
  //  LoopMBB:
  BB = LoopMBB;
  BuildMI(BB, DL, TII->get(Shakti::LOAD_SYNC), OldValue).addReg(Ptr).addImm(0);
  BuildMI(BB, DL, TII->get(Shakti::MOVESS), Dest).addReg(OldValue);

  unsigned NewValue;
  if (Opcode != 0) {
    // Perform an operation
    NewValue = MRI.createVirtualRegister(&Shakti::GPR32RegClass);
    ...
    if (InvertResult) {
      ...
      unsigned int Inverted = MRI.createVirtualRegister(&Shakti::GPR32RegClass);
      BuildMI(BB, DL, TII->get(Shakti::XORSSI), Inverted)
          .addReg(NewValue)
          .addImm(-1);
      NewValue = Inverted;
    }
  ...
  BuildMI(BB, DL, TII->get(Shakti::STORE_SYNC), Success)
      .addReg(NewValue)
      .addReg(Ptr)
      .addImm(0);
  BuildMI(BB, DL, TII->get(Shakti::BZ)).addReg(Success).addMBB(LoopMBB);
  BB->addSuccessor(LoopMBB);
...
MachineBasicBlock *
ShaktiTargetLowering::EmitAtomicCmpSwap(MachineInstr &MI,
                                       MachineBasicBlock *BB) const {
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &RegInfo = MF->getRegInfo();
  ...
  BB = Loop1MBB;
  BuildMI(BB, DL, TII->get(Shakti::LOAD_SYNC), Dest).addReg(Ptr).addImm(0);
  BuildMI(BB, DL, TII->get(Shakti::SNESISS), CmpResult)
      .addReg(Dest)
      .addReg(OldVal);
  BuildMI(BB, DL, TII->get(Shakti::BNZ)).addReg(CmpResult).addMBB(ExitMBB);
  ...
  BuildMI(BB, DL, TII->get(Shakti::STORE_SYNC), Success)
      ...
      .addReg(Ptr)
      .addImm(0);
  BuildMI(BB, DL, TII->get(Shakti::BZ)).addReg(Success).addMBB(Loop1MBB);
