//===- ShaktiDisassembler.cpp - Disassembler for Shakti ---------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file is part of the Shakti Disassembler.
//
//===----------------------------------------------------------------------===//

#define DEBUG_TYPE "shakti-disassembler"
...
class ShaktiDisassembler : public MCDisassembler {
public:
  ShaktiDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx,
...
static MCDisassembler *createShaktiDisassembler(const Target &T,
                                               const MCSubtargetInfo &STI,
                                               MCContext &Ctx) {
  return new ShaktiDisassembler(STI, Ctx, T.createMCRegInfo(""));
}

extern "C" void LLVMInitializeShaktiDisassembler() {
  // Register the disassembler.
  TargetRegistry::RegisterMCDisassembler(TheShaktiTarget,
                                         createShaktiDisassembler);
}

#include "ShaktiGenDisassemblerTables.inc"
...
DecodeStatus ShaktiDisassembler::getInstruction(MCInst &instr, uint64_t &Size,
...
static unsigned getReg(const void *D, unsigned RC, unsigned RegNo) {
  const ShaktiDisassembler *Dis = static_cast<const ShaktiDisassembler *>(D);
...
DecodeStatus DecodeGPR32RegisterClass(MCInst &Inst, unsigned RegNo,
  ...
  Inst.addOperand(
      MCOperand::createReg(getReg(Decoder, Shakti::GPR32RegClassID, RegNo)));
  return MCDisassembler::Success;
}
...
DecodeStatus DecodeVR512RegisterClass(MCInst &Inst, unsigned RegNo,
  ...
  // The internal representation of the registers counts r0: 1, r1: 2, etc.
  Inst.addOperand(
      MCOperand::createReg(getReg(Decoder, Shakti::VR512RegClassID, RegNo)));
  return MCDisassembler::Success;
}
